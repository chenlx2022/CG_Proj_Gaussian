...
__device__ glm::vec3 computeColorFromSH(int idx, int deg, int max_coeffs, const glm::vec3* means, glm::vec3 campos, const float* shs, bool* clamped)
{
	...
	*/
	...
	// Structure change: Evaluate SH basis functions into a local array, then compute dot product.
	float basis[16];
...
__device__ float3 computeCov2D(const float3& mean, float focal_x, float focal_y, float tan_fovx, float tan_fovy, const float* cov3D, const float* viewmatrix)
{
	...
	*/
	...
	// Calculate Jacobian J elements (sparse)
	float J00 = focal_x / t.z;
	float J02 = -(focal_x * t.x) / (t.z * t.z);
	float J11 = focal_y / t.z;
	float J12 = -(focal_y * t.y) / (t.z * t.z);

	// Get rotation matrix W from viewmatrix (columns of W are rows of R_world2cam)
	// Viewmatrix is assumed col-major, so viewmatrix[0,4,8] is the first row of the rotation.
...
__device__ void computeCov3D(const glm::vec3 scale, float mod, const glm::vec4 rot, float* cov3D)
{
	...
	*/
	...
	// Structure change: explicit column vectors and dot products
	...
	// Columns of M = S * R
	// Since S is diagonal, Row i of M is scaled by s_i.
	float3 m0 = { sx * r00, sy * r10, sz * r20 };
	float3 m1 = { sx * r01, sy * r11, sz * r21 };
	float3 m2 = { sx * r02, sy * r12, sz * r22 };

	// Sigma = M^T * M
	// This means Sigma_ij = dot(Column_i(M), Column_j(M))

	cov3D[0] = m0.x * m0.x + m0.y * m0.y + m0.z * m0.z;
	cov3D[1] = m0.x * m1.x + m0.y * m1.y + m0.z * m1.z;
	cov3D[2] = m0.x * m2.x + m0.y * m2.y + m0.z * m2.z;
	cov3D[3] = m1.x * m1.x + m1.y * m1.y + m1.z * m1.z;
	cov3D[4] = m1.x * m2.x + m1.y * m2.y + m1.z * m2.z;
	cov3D[5] = m2.x * m2.x + m2.y * m2.y + m2.z * m2.z;
}
